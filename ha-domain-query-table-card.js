 ...
 *
 * CHANGE requested:
 * - In TABLE mode, move "Domain: <domain> (<count>)" from header row 1 to header row 2.
 * - Keep swap button on row 1 (right aligned).
 * - Keep row 2 right-aligned filters, but now also includes domain text on the left.
 */

import { LitElement, html, css, nothing } from "https://unpkg.com/lit@2.8.0/index.js?module";
import "./ha-domain-query-table-card-editor.js";
...
class HaDomainQueryTableCard extends LitElement {
  ...
  _renderTableHeaderRow2(domain, totalCount) {
    // NEW: row 2 has left domain text, right filters
    return html`
  ...
  _renderTable(domain) {
    ...
    return html`
      <div class="header header-table">
        <!-- Row 1 now only contains the right aligned swap button -->
        <div class="header-row1">
          <div class="row1-left"></div>
          <div class="header-right">${this._renderSwapButton()}</div>
        </div>

        <!-- Row 2 contains Domain + filters -->
